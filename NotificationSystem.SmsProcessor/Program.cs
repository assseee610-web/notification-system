...
Console.WriteLine("[SmsProcessor] Запуск...");
...
var queueName = configuration["RabbitMQ:SmsQueue"];
...
consumer.ReceivedAsync += async (sender, args) =>
{
    try
    {
        ...
        if (isSuccess)
        {
            ...
            Console.WriteLine($"[SmsProcessor] SMS отправлен успешно!");
        }
        else
        {
            ...
            historyCmd.Parameters.AddWithValue("hid", Guid.NewGuid());
            historyCmd.Parameters.AddWithValue("nid", notificationId);
            historyCmd.Parameters.AddWithValue("err", "Ошибка отправки SMS (имитация)");
            historyCmd.Parameters.AddWithValue("now", DateTime.UtcNow);
            await historyCmd.ExecuteNonQueryAsync();

            Console.WriteLine($"[SmsProcessor] Ошибка SMS (попытка {retryCount}/{maxRetries})");
        }
    ...
    {
        Console.WriteLine($"[SmsProcessor] Ошибка: {ex.Message}");
        await channel.BasicNackAsync(args.DeliveryTag, false, true);
    }
};

await channel.BasicConsumeAsync(queue: queueName, autoAck: false, consumer: consumer);
Console.WriteLine($"[SmsProcessor] Подписан на '{queueName}'. Ожидание...");
